...
class ParentRegistrationController extends Controller
{
    ...
    private function insertStudentField($school_id){
        ...
        return SmStudentField::where('school_id', $school_id)->get();
    }
    ...
    public function registration($page_link)
    {
        ...
        try {
                ...
                $fields=SmStudentField::where('active_status', 1) ->where('school_id', $school->id)->get();
    ...
    public function getClasses(Request $request)
    {
        ...
        if (isset($request->school_id)) {
            $classes = SmClass::where('active_status', '=', '1')->where('academic_id', $request->id)->where('school_id', $request->school_id)->get();
        } else {
            $school = app('school');
    ...
    public function studentStore(SmStudentRegistrationRequest $request)
    {
         ...
         //remove for add validation request ->abu nayem
            // return $request->all();
        ...
        try {
            if($request->relationButton=='F'){
                $relation='Father';
                $guardians_photo=OnlineRegistrationFileUpload('public/uploads/student/',$request->file('fathers_photo'));
            }elseif($request->relationButton=='M'){
                $relation='Mother';
                $guardians_photo=OnlineRegistrationFileUpload('public/uploads/student/',$request->file('mothers_photo'));
            }else{
                $relation='Other';
                $guardians_photo=OnlineRegistrationFileUpload('public/uploads/student/',$request->file('guardians_photo'));
            }
            ...
            $student->student_mobile = $request->phone_number;
            ...
            $setting = SmRegistrationSetting::find(1);
                ...
                try{
                     $reciver_email = $school->email;
                     $receiver_name =  $school->school_name;
                     $subject= $school_admin['school_name'] . ' '. 'Have New Registration';
                     $view ="parentregistration::school_admin_email";
                     $compact['compact'] =  array('email' => $school->email,'school_name' => $school->school_name);
