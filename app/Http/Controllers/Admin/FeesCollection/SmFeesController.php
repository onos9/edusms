            ...
            if($request->send_email){
                ...
                foreach($request->student_list as $student){
                    ...
                    if($student_detail->student->mobile != ""){
                        ...
                        $compact['id'] = $student_detail->student->id;
                        $compact['student_name'] = $student_detail->student->full_name;
                        @send_sms($student_detail->student->mobile, 'student_fees_due', $compact);
                    }
                    if($student_detail->student->parents->guardians_mobile != ""){
                        $compact['slug'] = 'parent';
                        $compact['id'] = $student_detail->student->parents->id;
                        $compact['parent_name'] = $student_detail->student->parents->guardians_name;
                        @send_sms($student_detail->student->parents->guardians_mobile, 'student_fees_due_for_parent', $compact);
                    }
                }
            }
            Toastr::success('Operation successful', 'Success');
