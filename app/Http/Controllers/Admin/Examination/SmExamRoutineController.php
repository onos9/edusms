    ...
    public function addExamRoutineStore(Request $request)
    {
        ...
        try {
            ...
            foreach ($request->routine as $routine_data) {
                ...
                if ($is_exist) {
                    $allStudents = StudentRecord::with('student')->where('class_id', $class_id)
                            ->where('section_id', $section_id)
                            ->get();
                    $examType = SmExamType::find($request->exam_type_id);
                    foreach($allStudents as $allStudent){
                        $compact['user_email'] = $allStudent->student->email;
                        $compact['student_name'] = $allStudent->student->full_name;
                        $compact['parent_name'] = $allStudent->student->parents->guardians_name;
                        ...
                        $number = $allStudent->student->mobile;
                        $parentNumber = $allStudent->student->parents->guardians_mobile;
