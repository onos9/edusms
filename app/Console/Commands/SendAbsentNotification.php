                ...
                if ($setup) {
                    ...
                    if ($is_school_day) {
                        ...
                        foreach (array_unique($students) as $key => $student_id) {
                            ...
                            if($attendanceInfo && $attendanceInfo->recordDetail->student->parents){
                                $absent_subjects=SmSubjectAttendance::getAbsentSubjectList($student_id, $school->id);
                                $compact['user_email'] = @$attendanceInfo->recordDetail->student->parents->guardians_email;
                                $compact['subject_list'] = implode(',',$absent_subjects);
                                $compact['number_of_subject'] = count($absent_subjects);
                                $compact['date'] = date('Y-m-d');
                                $compact['slug'] = 'parent';
                                $compact['id'] = @$attendanceInfo->recordDetail->student->parents->id;
                                $compact['student_name'] = @$attendanceInfo->recordDetail->student->full_name;
    
                                @send_sms(@$attendanceInfo->recordDetail->student->parents->guardians_mobile, 'student_absent_notification', $compact, $school->id);
