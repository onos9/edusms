...
class SmApiController extends Controller
{
    ...
    public function UpdateStudentApi(Request $request)
    {
        ...
        try {
                $request_id = $request->id;
                $student = SmStudent::find($request_id);
                ...
                if ($request->file('doc') != "" && $request->title != "") {
                    ...
                    $document->student_staff_id = $request_id;
                    $document->type = 'stu';
    ...
    public function studentDetails(Request $request)
    {
        try {
            $students = SmStudent::where('sm_students.active_status', 1)
                ->join('student_records','sm_students.id','=', 'student_records.student_id')
                ->join('sm_classes', 'sm_classes.id', '=', 'student_records.class_id')
                ->join('sm_sections', 'student_records.section_id', '=', 'sm_sections.id')
                ->join('sm_academic_years', 'sm_academic_years.id', '=', 'student_records.session_id')
                ->join('sm_general_settings', 'sm_students.school_id','=','sm_general_settings.school_id');
                
            $students->where('sm_students.academic_id', SmAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR());
            $students->with(array('parents' => function ($query) {
                $query->select('id', 'fathers_name', 'mothers_name','guardians_email');
            }));
            
            $students->with(array('gender' => function ($query) {
                    $query->select('id', 'base_setup_name');
            }));
            
            $students->select('sm_students.*','student_records.class_id','student_records.session_id','sm_classes.class_name','sm_sections.section_name','sm_academic_years.year','sm_general_settings.school_code');
    ...
    public function studentDetailsSearch(Request $request)
    {
        ...
        try {
            ...
            if ($request->class != "") {
                $students->where('sm_classes.class_name', $request->class);
            }
            if ($request->section != "") {
                $students->where('sm_sections.section_name', $request->section);
            }
            if ($request->name != "") {
    ...
    public function studentView(Request $request, $id)
    {
        try {
            ...
            $students->select('sm_students.*','student_records.class_id','student_records.session_id','sm_classes.class_name','sm_sections.section_name','sm_academic_years.year');
            $students->where('sm_students.id', $id);
            $student = $students->firstOrFail();

            if (ApiBaseMethod::checkUrl($request->fullUrl())) {
                $data = [];
                $data['rStudent'] = $student->toArray();
    ...
    public function assignSubject(Request $request)
    {
        ...
        try {
            $assign_subjects = SmAssignSubject::with('class', 'section')->status()->orderBy('class_id', 'ASC')->orderBy('section_id', 'ASC')->get();
