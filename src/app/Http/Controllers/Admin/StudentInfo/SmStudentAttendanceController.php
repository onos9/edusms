        ...
        try {
            ...
            foreach ($request->attendance as $record_id => $student) {
                ...
                if(gv($student, 'attendance_type') == "P"){
                    $compact['user_email'] = $studentInfo->student->email;
                    $compact['student_name'] = $studentInfo->student->full_name;
                    @send_sms($studentInfo->student->mobile, 'student_attendance', $compact);

                    $compact['user_email'] = $studentInfo->student->parents->guardians_email;
                    $compact['parent_name'] = $studentInfo->student->parents->guardians_name;
                    @send_sms($studentInfo->student->parents->guardians_mobile, 'student_attendance_for_parent', $compact);
                }elseif(gv($student, 'attendance_type') == "L"){
                    $compact['user_email'] = $studentInfo->student->email;
                    $compact['student_name'] = $studentInfo->student->full_name;
                    @send_sms($studentInfo->student->mobile, 'student_late', $compact);

                    $compact['user_email'] = $studentInfo->student->parents->guardians_email;
                    $compact['parent_name'] = $studentInfo->student->parents->guardians_name;
                    @send_sms($studentInfo->student->parents->guardians_mobile, 'student_late_for_parent', $compact);
                }elseif(gv($student, 'attendance_type') == "A"){
                    $compact['user_email'] = $studentInfo->student->email;
                    $compact['student_name'] = $studentInfo->student->full_name;
                    @send_sms($studentInfo->student->mobile, 'student_absent', $compact);

                    $compact['user_email'] = $studentInfo->student->parents->guardians_email;
                    $compact['parent_name'] = $studentInfo->student->parents->guardians_name;
                    @send_sms($studentInfo->student->parents->guardians_mobile, 'student_absent_for_parent', $compact);
                }
